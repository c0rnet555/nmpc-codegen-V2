...
static int nmpc_prepare(real_t* static_casadi_parameters,const real_t* current_state,const real_t* state_reference,\
        const real_t* input_reference,const real_t* optimal_inputs,const real_t* obstacle_rect,const real_t* obstacle_circle);
...
int nmpc_init(void){
    ...
    #else
        static_casadi_parameters = malloc((2*DIMENSION_STATE+DIMENSION_INPUT+DIMENSION_RECT+DIMENSION_CIRCLE)*sizeof(real_t));
    #endif
...
static int nmpc_prepare(real_t* static_casadi_parameters,const real_t* current_state,const real_t* state_reference,\
    ...
    int offset = 2*DIMENSION_STATE;
    for(i=0;i<DIMENSION_INPUT;i++){
        static_casadi_parameters[offset++] = input_reference[i];
    }
    for(i=0;i<DIMENSION_RECT;i++){
        static_casadi_parameters[offset++] = obstacle_rect[i];
    }
    for(i=0;i<DIMENSION_CIRCLE;i++){
        static_casadi_parameters[offset++] = obstacle_circle[i];
    }
...
int npmc_solve( const real_t* current_state,
		        ...
		        const real_t* obstacles_rect,
		        const real_t* obstacles_circ,
                const real_t* obstacles_nonconvex,
		        const real_t* obstacles_convex
		        ){
     ...
     */
    nmpc_prepare(static_casadi_parameters,current_state,state_reference,input_reference,optimal_inputs,obstacles_rect,obstacles_circ);
    #ifdef USE_LA
